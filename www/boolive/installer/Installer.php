    ...
    static function start()
    {
        if(empty($_SERVER['HTTP_X_REQUESTED_WITH']) || strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest'){
            ...
            try{
                ...
                if (is_dir(DIR_TEMP)){
                    File::clearDir(DIR_TEMP);
                }
                $modules_list = array(
                    'engine' => self::sortClasses(self::scanClasses(DIR.'boolive/')),
                    'project' => self::scanInfo(DIR.'site/')
                );
...
//                trace($modules_list['project']);
    ...
    {
		ob_start();
			$file = DIR.'boolive/installer/tpl/'.$template.'.php';
			if (file_exists($file)){
				include $file;
			}else{
		...
		foreach ($dirs as $d){
            if (is_dir($dir.$d)){
                ...
                if (substr($d, strlen($d)-5)=='.info'){
                    ...
                    $info['uri'] = trim(preg_replace('#\\\\#u','/', mb_substr($dir, mb_strlen(DIR.'site'))), '/\\');
    ...
    static function installStep()
    {
        ...
        if ($result['step_cnt'] == 0 || $install['step'] >= $result['step_cnt']){
            ...
            $result['percent'] = sprintf('%d', $install['step'] / $result['step_cnt'] * 100);
        }
