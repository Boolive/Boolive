...
class Role extends Entity
{
    ...
    {
        $roles = $this->$action_kind->find(array('comment' => 'read role action conditions'), false, true, false);
        $curr = null;
        $prev = null;
        $cond = null;
        ...
        foreach ($roles as $r){
            if (($c = $r->getAccessCond($action_kind, $parent, $depth)) && is_array($c)){
                ...
                if ($curr!=$need){
                    ...
                    if ($need == 'all' && $c[0] == 'not' && count($c[1]) == 2 && $c[1][0] == 'of' && $prev[0] == 'not' && count($prev[1]) == 2 && $prev[1][0] == 'of'){
