...
class Events
{
     ...
     */
    public static function RemoveHandler($event_name, $handler_module, $handler_method)
    {
     ...
     */
    public static function Send($event_name, $params = array())
    {
        $r = new EventResult();
        if (isset(self::$handlers[$event_name])) {
            ...
            for ($i = 0; $i < $cnt; $i++) {
                ...
                if (method_exists(self::$handlers[$event_name][$i][0],
                    self::$handlers[$event_name][$i][1])
                ) {
                    $value = call_user_func_array(array(self::$handlers[$event_name][$i][0],
                        self::$handlers[$event_name][$i][1]
                    ), $params);
                    if (isset($value)) {
                        $r->value = $value;
                    ...
                    if (!empty(self::$handlers[$event_name]['once'])) {
                        self::RemoveHandler($event_name,
                            self::$handlers[$event_name][$i][0],
                            self::$handlers[$event_name][$i][1]);
                    }
                }
            }
